...
class CNNQNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.fc1 = layer_init(nn.Linear(128 + 1 + act_shape[0], 128))
        self.fc2 = layer_init(nn.Linear(128, 1))

    def forward(self, obs, act):
        obs_sig = obs[:, :, :-1]
        obs_ite = obs[:, :, -1:]
        obs_sig_encoding = F.relu(self.obs_encoder(obs_sig))
        x = torch.cat([obs_sig_encoding, obs_ite.squeeze(2), act], dim=1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
class CNNActor(nn.Module):
    def __init__(self, env):
        ...
        self.obs_encoder = Encoder(obs_shape, 128)
        self.fc1 = layer_init(nn.Linear(128 + 1, 128))
        self.fc_mean = layer_init(nn.Linear(128, act_shape[0]))
        self.fc_log_std = layer_init(nn.Linear(128, act_shape[0]))
